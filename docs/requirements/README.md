...
**Live Implementation honesty:** Notes must name live helpers (`main_spring_boot_app`, `output_text`, `perform_self_install`, `util_resolve_storage`, …). Live families are product law (`output_*`, util/lifecycle/domain). Bootstrap seed `out_*`/`inst_*`/`app_*` is **not** required on this product (§3.1 option 2). Storage: `util_resolve_storage` law in **`requirement-shell-cli-storage`** (helper present; call-site Gap until consumers land). Installed empty argv = domain run.
