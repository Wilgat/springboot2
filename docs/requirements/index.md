...
**Identity SSOT:** ship unit `./springboot2` Project Constants — `APP_NAME="springboot2"`, `VERSION="2.3.2"`, `REPO_USER="Wilgat"`, `REPO_NAME="springboot2"`, `SCRIPT_URL` composed from those. Requirements **must not** invent a different product name, channel, or version.  
**Workspace state:** Specialized product law — identity SSOT retargeted; Implementation Notes use **live** `./springboot2` helpers; class law Active (`requirement-class-software-dev`); domain law registered (`requirement-domain-springboot2`).  
...
**Sufficiency note:** Class + domain + storage + integrity law registered; help↔dispatcher / force / hybrid empty-argv / Shape A+B checksum / tests green; Implementation Notes re-synced 2026-07-15. Storage: resolve+mkdir fail-closed, `EFFECTIVE_STORAGE_DIR` + `TMPDIR` in main, about fields. Residual: optional downgrade JSON code wording; JSON error on stdout vs stderr.  
**Updated:** 2026-08-10 (class law; domain rename `requirement-domain-*`; TP-MOD; docs inventory)

| ID / key | Title | Area | Status | Path | Updated |
|----------|-------|------|--------|------|---------|
| requirement-class-software-dev | Software-development class law + residual stack (bash Type O-P + domain pointers) | class | Active | `requirement-class-software-dev.md` | 2026-08-10 |
...
| requirement-shell-cli-interface | Shell CLI interface (commands, flags, dispatch, modes) | shell | Active | `requirement-shell-cli-interface.md` | 2026-07-20 |
...
| requirement-shell-cli-zero-arguments | Empty argv Type O-P combined ensure (ship unit + payload) | shell | Active | `requirement-shell-cli-zero-arguments.md` | 2026-07-24 |
...
| requirement-shell-interactive-vs-noninteractive | Interactive vs non-interactive / `curl\|bash` behavior | shell | Active | `requirement-shell-interactive-vs-noninteractive.md` | 2026-07-24 |
...
| requirement-shell-payload-online-install | Type O-P payload online installer (combined ensure; install/uninstall vs self-*) | shell | Active | `requirement-shell-payload-online-install.md` | 2026-07-24 |
...
| requirement-domain-springboot2 | Spring Boot payload content (SDKMAN/Java/Maven/Boot 2.7.18, project preserve, build/run) | domain | Active | `requirement-domain-springboot2.md` | 2026-08-10 |
