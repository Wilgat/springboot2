...
| **C. File I/O (redirected)** | `printf … >> "$file"` that appends config/content to a path is file mutation, not product stdout/stderr messaging. User-visible “what changed” lines still go through Output SSOT (`out_*`). | `path_add_bashrc`, `path_add_zshrc`, `path_add_fish` |
| **D. Tool protocol / computation pipes** | `printf` / tool stdout for digests only; product status still via Output SSOT. | `util_sha256_file` → `sha256sum`/`shasum`/`openssl` capture; integrity messages via `util_verify_download_integrity` |
...
- Extra diagnostics **MUST** go through the output system (or a dedicated debug via `out_text` when DEBUG=1 (if present) when added) on **stderr**.  
...
| Item | Status |
|------|--------|
| Quiet keeps `out_warn` + `out_error` on stderr | **Gap / product choice** — quiet suppresses non-error levels in `out_text` |
| JSON errors on stderr via `out_json_error` | **Gap** — JSON helpers write **stdout** only |
| Debug when DEBUG=1 | **Gap** — limited; no full `out_debug` / `--debug` surface |
| Command paths use human **or** JSON via mode flags | **Partial** — `out_text` short-circuits when `JSON=1`; JSON emitters on stdout |
...
- **Pair with modular design:** Only Output SSOT (`out_*`) owns emission (`requirement-shell-modular-function-design.md`).  
...
|----------------|-------|--------|
| **TP-CLI-04,07** | `tests/test_cli.sh` | have |
