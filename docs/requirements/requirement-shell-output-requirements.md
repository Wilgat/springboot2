...
| **D. Tool protocol / computation pipes** | `printf` / tool stdout for digests only; product status still via Output SSOT. | `file_sha256` → `sha256sum`/`shasum`/`openssl` capture; integrity messages via `verify_download_integrity` |
